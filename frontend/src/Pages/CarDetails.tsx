...
// Reservation data is handled directly without explicit interface

// Removed duplicate function definition. Use only the typed CarDetails component below.

const CarDetails: React.FC<{ showNotification?: (msg: string, type?: string) => void }> = ({ showNotification }) => {
  ...
  const handleSaveCar = async () => {
    if (!carData) return;
    if (!isAuthenticated) {
      if (showNotification) showNotification(langCtx?.translate.saveCarErrorNoAuth ?? "Error", "error");
      return;
    }
    try {
      const userId = user?.id;
      if (!userId) {
        if (showNotification) showNotification(langCtx?.translate.saveCarErrorNoUser ?? "Error", "error");
        return;
      }
      await SavedCarService.saveCar(userId, carData.id);
      if (showNotification) showNotification(langCtx?.translate.saveCarSuccess ?? "Success", "success");
    } catch (error) {
      console.error("Hiba az autó mentésekor:", error);
      if (showNotification) showNotification(langCtx?.translate.saveCarError ?? "Error", "error");
    }
  };
  ...
  return (
      ...
      <div className="details">
        ...
        <p id="car-dimensions">
          {langCtx?.translate.dimensions}: {carData.carModel.length} x {carData.carModel.width} x {carData.carModel.height} mm
        </p>
        ...
        <p id="car-color">
          {langCtx?.translate.color} {carData.color.colorNameEnglish}
        </p>
